...
class GeustsListController: UIViewController {
    ...
    private func setupUI(){
        ...
        tableView.addRefreshControl(target: self, action: #selector(refresh))
 
    }
    
    
    @objc func refresh(){
        getDataFromService(isLoading: false)
    }
    ...
    func getDataFromService(isLoading: Bool = true){
        if isLoading{
        ...
        GuestsViewModel.shared.getGuestsList { response in
            DispatchQueue.main.async {
                self.guestsList = response.results.reversed()
            }
        }
    }
}
